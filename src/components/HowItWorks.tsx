...
import { MessageSquareCheck, Settings2, Sparkles, CheckCircle2 } from 'lucide-react';
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {steps.map((item, idx) => {
          ...
          return (
            <div
              key={idx}
              className="glass-panel p-8 rounded-2xl border border-white/10 hover:border-primary/50 transition-all space-y-4 relative flex flex-col justify-between"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-3xl font-extrabold text-white/20 font-mono">{item.step}</span>
                </div>

                <h3 className="text-xl font-bold text-on-surface leading-snug">{item.title}</h3>
                <p className="text-xs sm:text-sm text-on-surface-variant leading-relaxed">{item.description}</p>
              </div>
            </div>
          );
        })}
